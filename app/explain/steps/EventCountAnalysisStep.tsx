...
import { EventCountAnalysisStep as EventCountAnalysisStepType } from "./types";
import { useHighlightIds, useErrorIds } from "./HighlightIdsContext";

interface EventCountAnalysisStepProps {
  step: EventCountAnalysisStepType;
}

export default function EventCountAnalysisStep({
  step,
}: EventCountAnalysisStepProps) {
  const highlightIds = useHighlightIds();
  const errorIds = useErrorIds();
  const count = step.eventCount.value;
  const isHighlighted = highlightIds.has(step.eventCount.id);
  const isError = errorIds.has(step.eventCount.id);
