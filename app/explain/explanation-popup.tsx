...
import { lookupCitationCell, highlightEntityInSteps, findEntityValue } from "../utils/citationIds";
import DebugView from "./debug-view";
import { StepRenderer, Step, PrefixedIdGenerator } from "./steps";
import { createComparisonStep } from "./steps/convertToSteps";
import { HighlightIdsProvider } from "./steps/HighlightIdsContext";
...
/**
 * Parse operators from a reference string (excluding comma)
 */
function parseOperators(reference: string): string[] {
              ...
              <div className="flex flex-col gap-1 min-w-max pr-1">
                {cellGroupsWithSteps.map((group, groupIdx) => (
                  <div key={groupIdx}>
                    ...
                    <HighlightIdsProvider highlightIds={group.highlightIds} errorIds={popupData.traceIds?.errorId ? parseReference(popupData.traceIds.errorId).ids : undefined}>
